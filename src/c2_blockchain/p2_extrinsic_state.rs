...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        for header in chain.iter() {
            if header.parent != parent_hash || header.height != height + 1 {
                return false;
            }
            if parent.state + header.extrinsic != header.state {
                return false;
            }
...
fn build_valid_chain(n: u64) -> Vec<Header> {
    let mut b = Header::genesis();
    let mut chain_vec = Vec::default();
    for _ in 0..n {
        chain_vec.push(b.clone());
        b = b.child(3);
    }
    chain_vec
}
...
fn build_forked_chain() -> (Vec<Header>, Vec<Header>) {
    ...
    (vec![g.clone(), b1, b2], vec![g.clone(), c1, c2])
}
